    ...
    if (httr::status_code(response) == 200) {
      ...
      if (nchar(raw_content) > 0) {
        ...
        if ("dataElements" %in% names(subject_structure)) {
          ...
          # Optional: Interactive pager to browse/add recommended elements
          if (nrow(all_recommended_elements) > 0) {
            rec_names <- all_recommended_elements$name
            page_size <- 5
            total <- length(rec_names)
            page <- 1
            repeat {
              start_idx <- (page - 1) * page_size + 1
              end_idx <- min(page * page_size, total)
              cat(sprintf("\nRecommended elements %d-%d of %d (press Enter to continue):\n", start_idx, end_idx, total))
              for (i in seq(start_idx, end_idx)) {
                local_num <- i - start_idx + 1
                name_i <- rec_names[i]
                marker <- ifelse(name_i %in% recommended_elements$name, "[included]", "")
                message(sprintf("%d. %s %s", local_num, name_i, marker))
              }
              prompt_msg <- "Select 1-5 to add, 'n' next, 'p' prev, 'm' manual add, Enter to finish, Esc to quit: "
              ans <- tryCatch({
                readline(prompt = prompt_msg)
              }, interrupt = function(e) {
                message("Interactive selection cancelled (Esc pressed). Continuing...")
                ""  # treat as finish
              })
              if (ans == "") break
              if (tolower(ans) == "n") { if (end_idx < total) page <- page + 1 else message("Already at last page"); next }
              if (tolower(ans) == "p") { if (page > 1) page <- page - 1 else message("Already at first page"); next }
              if (tolower(ans) == "m") {
                manual <- readline(prompt = "Enter recommended element name to add (exact), or Enter to cancel: ")
                if (nzchar(manual)) {
                  if (manual %in% all_recommended_elements$name) {
                    # append to recommended_elements if not already
                    if (!(manual %in% recommended_elements$name)) {
                      recommended_elements <- rbind(recommended_elements, all_recommended_elements[all_recommended_elements$name == manual, ])
                      message(sprintf("Added '%s' to recommended elements list", manual))
                      # If not in df, create empty column of correct type
                      if (!(manual %in% names(df))) {
                        col_type <- all_recommended_elements$type[all_recommended_elements$name == manual][1]
                        if (grepl("String|GUID", col_type, ignore.case = TRUE)) {
                          df[[manual]] <- NA_character_
                        } else if (grepl("Integer", col_type, ignore.case = TRUE)) {
                          df[[manual]] <- NA_integer_
                    ...
                    message("Name not found in ndar_subject01 recommended list")
                  }
                }
                next
              }
              if (grepl("^[0-9]+$", ans)) {
                sel_local <- as.numeric(ans)
                if (!is.na(sel_local) && sel_local >= 1 && sel_local <= (end_idx - start_idx + 1)) {
                  global_index <- start_idx + sel_local - 1
                  pick <- rec_names[global_index]
                  if (!(pick %in% recommended_elements$name)) {
                    recommended_elements <- rbind(recommended_elements, all_recommended_elements[all_recommended_elements$name == pick, ])
                    message(sprintf("Added '%s' to recommended elements list", pick))
                    # If not in df, create empty column of correct type
                    if (!(pick %in% names(df))) {
                      col_type <- all_recommended_elements$type[all_recommended_elements$name == pick][1]
                      if (grepl("String|GUID", col_type, ignore.case = TRUE)) {
                        df[[pick]] <- NA_character_
                      } else if (grepl("Integer", col_type, ignore.case = TRUE)) {
                        df[[pick]] <- NA_integer_
                      } else if (grepl("Float", col_type, ignore.case = TRUE)) {
                        df[[pick]] <- NA_real_
